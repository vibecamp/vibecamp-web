...
export default function register(router: Router) {
  ...
  defineRoute(router, {
    ...
    handler: async ({
      jwt: { account_id },
      body: { email_address: raw_email_address },
    }) => {
  ...
  defineRoute(router, {
    ...
    handler: async ({
      jwt: { account_id },
      body: { attendee_id, associated_account_id: _, ...attendeeUpdate },
    }) => {
      const attendee = await withDBConnection(
        async (db) =>
          (
      ...
      async ({ jwt, body: { invite_code } }) => {
        ...
        return await withDBTransaction(async (db) => {
          const inviteCodeResult = (
            await db.queryTable("invite_code", {
          ...
          const codeUsedByCurrentAccount = (
            await db.queryTable("invite_code", {
