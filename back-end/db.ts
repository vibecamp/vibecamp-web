...
import { MAX_ADULT_TICKETS_PER_ACCOUNT, REFERRAL_MAXES } from "common/constants.ts";
...
export async function accountReferralStatus(db: Pick<Transaction, "queryObject" | "queryArray">, account_id: number): Promise<{ allowedToRefer: number, allowedToPurchaseTickets: number }> {
    const res = await db.queryObject<{ account_id: number, is_seed_account: boolean, created_by_account_id: Maybe<number> }>`
    ...
    return { allowedToRefer: REFERRAL_MAXES[referralDistance] ?? 0, allowedToPurchaseTickets: MAX_ADULT_TICKETS_PER_ACCOUNT }
}

export async function fullAccountInfo(account_id: number): Promise<Maybe<FullAccountInfo>> {
    const { referralStatus: { allowedToPurchaseTickets }, accounts, attendees, tickets } = await withDBTransaction(async db => {
...
export async function useInviteCode(account_id: number, invite_code: string): Promise<boolean> {
    try {
