...
export default observer(() => {
  ...
  return (
    <Col
      padding={20}
      pageLevel
      justify={loadingOrError ? "center" : undefined}
      align={loadingOrError ? "center" : undefined}
    >
      {loading ? (
        <LoadingDots size={100} color="var(--color-accent-1)" />
      ) : Store.accountInfo.state.kind === "error" ||
        Store.accountInfo.state.result == null ? (
        "Failed to load"
      ) : Store.accountInfo.state.kind === "result" ? (
        <>
          <h1 style={{ fontSize: 24, alignSelf: "flex-start" }}>My tickets</h1>
          ...
          {Store.festivals.state.result?.map((festival) => {
            const tickets =
              Store.purchasedTicketsByFestival[festival.festival_id] ?? [];
            const otherPurchases =
              Store.nonTicketPurchasesByFestival[festival.festival_id] ?? [];
            const cabinName =
              festival.festival_id === "4821bd6a-9e16-4944-b9a1-afe3256ff18d"
                ? Store.vibecampCabin
                : null;
            ...
            return (
              <div
                ...
                style={
                  festival.end_date.isBefore(dayjs.utc())
                    ? { opacity: 0.5 }
                    : undefined
                }
              >
                ...
                {tickets.map((ticket, index) => (
                  <React.Fragment key={ticket.purchase_id}>
                    ...
                    <Ticket
                      name={undefined}
                      ticketType="adult"
                      ownedByAccountId={ticket.owned_by_account_id}
                    />
                  </React.Fragment>
                ))}

                {cabinName !== null && cabinName.state.result !== null ? (
                  cabinName.state.result === "" ? (
                    <React.Fragment key="cabin_name">
                      <div>No Cabin</div>
                    </React.Fragment>
                  ) : (
                    <React.Fragment key="cabin_name">
                      Cabin: {cabinName.state.result}
                    </React.Fragment>
                  )
                ) : null}
                        ...
                        {otherPurchases.map((p, i) => (
                          <div
                            style={{
                              padding: "4px 8px",
                              borderTop:
                                i > 0 ? "var(--controls-border)" : undefined,
                            }}
                            key={p.purchase_id}
                          >
                            1x{" "}
                            {
                              Store.purchaseTypes.state.result?.find(
                ...
                {festival.sales_are_open && (
                  <>
                    ...
                    <Button
                      isPrimary
                      onClick={openTicketPurchaseModal(festival.festival_id)}
                    >
                      {tickets.length === 0
                        ? "Buy tickets"
