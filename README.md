...
`.env.public` has a committed `DB_URL` that points to our test database. This is an anonymized clone of the production database, useful for safely testing code changes. If you need to point at the production database (and have access to it), you can create a `.env` (which is **not** committed to github) with its own `DB_URL`, which will override the one in `.env.public`.